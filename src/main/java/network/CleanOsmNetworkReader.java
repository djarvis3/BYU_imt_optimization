...
public class CleanOsmNetworkReader implements MatsimSomeReader{
	...
	private static final List<String> allTags = new LinkedList<>(Arrays.asList(TAG_LANES, TAG_LANES_FORWARD,
			TAG_LANES_BACKWARD, TAG_HIGHWAY, TAG_MAXSPEED, TAG_CAPACITY, TAG_JUNCTION, TAG_ONEWAY, TAG_ACCESS));
	...
	/*package*/ final List<org.matsim.core.utils.io.OsmNetworkReader.OsmFilter> hierarchyLayers = new ArrayList<>();
	 ...
	 */
	public CleanOsmNetworkReader(final Network network, final CoordinateTransformation transformation) {
	 ...
	 */
	public CleanOsmNetworkReader(final Network network, final CoordinateTransformation transformation, final boolean useHighwayDefaults) {
	 ...
	 *
	 */
	public CleanOsmNetworkReader(final Network network, final CoordinateTransformation transformation, final boolean useHighwayDefaults, final boolean useVspAdjustments) {
...
//			this.setHighwayDefaults(6, "living_street", 1,  15.0/3.6, 1.0,  300);
	 ...
	 * or branching are kept as nodes. This reduces the number of nodes and links in the network, but can in some rare
	 ...
	 * @param osmFilter
	 */
	public final void addOsmFilter(final org.matsim.core.utils.io.OsmNetworkReader.OsmFilter osmFilter) {
			...
			wayLoop: for (OsmWay way : node.ways.values()) {
				...
				if ((highway != null) && (this.highwayDefaults.containsKey(highway))) {
					if (this.hierarchyLayers.isEmpty()) {
						...
						for (org.matsim.core.utils.io.OsmNetworkReader.OsmFilter osmFilter : this.hierarchyLayers) {
	...
	private void createLink(final Network network, final OsmWay way, final OsmNode fromNode, final OsmNode toNode,
		...
		double laneCapacity_vehPerHour = defaults.laneCapacity_vehPerHour;
		...
		String capacityTag = way.tags.get(TAG_CAPACITY);
		if (capacityTag != null) {
				laneCapacity_vehPerHour = Double.parseDouble(capacityTag);
			}
		...
		if (maxspeedTag != null) {
			try {
				if(maxspeedTag.endsWith("mph")) {
					...
					freespeed = Double.parseDouble(maxspeedTag); // keep speed in// m/s
				}
				if (useVspAdjustments) {
		...
		try {
			...
			if (lanesForwardTag != null && lanesBackwardTag != null){
				...
				if (lanesTag != null) {
					 ...
					 */
					nofLanesForward = Double.parseDouble(lanesTag);
					/* Keep the number of lanes as is, DJ */
		...
		double capacityForward = nofLanesForward * laneCapacity_vehPerHour;
		double capacityBackward = nofLanesBackward * laneCapacity_vehPerHour;
	...
	private static class OsmFilterImpl implements org.matsim.core.utils.io.OsmNetworkReader.OsmFilter {
	...
	private static class GeographicallyNonrestrictingOsmFilterImpl implements org.matsim.core.utils.io.OsmNetworkReader.OsmFilter {
		private final int hierarchy;
	...
	protected static class OsmHighwayDefaults {
		...
		public final double laneCapacity_vehPerHour;
		...
		public OsmHighwayDefaults(final int hierarchy, final double lanesPerDirection, final double freespeed, final double freespeedFactor, final double laneCapacity, final boolean oneway) {
			...
			this.laneCapacity_vehPerHour = laneCapacity;
			this.oneway = oneway;
		}
	}
		...
		public void endTag(final String name, final String content, final Stack<String> context) {
			if ("way".equals(name)) {
				if (!this.currentWay.nodes.isEmpty()) {
					boolean used = false;
					OsmHighwayDefaults osmHighwayDefaults = CleanOsmNetworkReader.this.highwayDefaults.get(this.currentWay.tags.get(TAG_HIGHWAY));
					if (osmHighwayDefaults != null) {
						int hierarchy = osmHighwayDefaults.hierarchy;
						this.currentWay.hierarchy = hierarchy;
						if (CleanOsmNetworkReader.this.hierarchyLayers.isEmpty()) {
							used = true;
						}
						if (this.collectNodes) {
							used = true;
						} else {
							for (org.matsim.core.utils.io.OsmNetworkReader.OsmFilter osmFilter : CleanOsmNetworkReader.this.hierarchyLayers) {
