...
public class DecongestionTollingPID implements DecongestionTollSetting, LinkLeaveEventHandler {
	private static final Logger log = LogManager.getLogger(DecongestionTollingPID.class);
	...
	public void updateTolls() {
		...
		for (Id<Link> linkId : this.congestionInfo.getlinkInfos().keySet()) {
			...
			if (K_i != 0. && integralApproach.equals(DecongestionConfigGroup.IntegralApproach.UnusedHeadway.toString())) {
...
//				// Ideen:
				...
				if (K_i != 0.) {
					...
					if (integralApproach.equals(DecongestionConfigGroup.IntegralApproach.Average.toString())) {
					...
					if (linkId2time2totalDelayAllIterations.get(linkId) == null) {
						...
						if (this.linkId2time2totalDelayAllIterations.get(linkId).get(intervalNr) == null) {
							...
							if (averageDelay <= congestionInfo.getDecongestionConfigGroup().getToleratedAverageDelaySec()) {
								...
								} else if (integralApproach.equals(DecongestionConfigGroup.IntegralApproach.Zero.toString())) {
