...
public final class ActionCreator implements VrpAgentLogic.DynActionCreator {
	...
	public DynAction createAction(DynAgent dynAgent, DvrpVehicle vehicle, double now) {
		...
		return switch ((Optimizer.ImtTaskType) task.getTaskType()) {
			...
			// If the task type is DRIVE_TO_INCIDENT, return a VrpLegFactory object
			// that creates a leg for an IMT with an offline tracker
			case DRIVE_TO_INCIDENT -> VrpLegFactory.createWithOfflineTracker(TransportMode.truck, vehicle, timer);

			// If the task type is ARRIVAL, INCIDENT_MANAGEMENT, DEPARTURE, or WAIT,
			// return an IdleDynActivity object with the task type as its name and the task's end time as its end time
